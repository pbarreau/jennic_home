...
PUBLIC void vPRT_ManageRelays(uint8 conf)
{
  uint8 i = 0;
  uint32 Maconf = u32AHI_DioReadInput(); /// Lecture etat courant

  vPrintf("Relays request = %x\n", conf);

  // Ne Modifier que les bits des relais
  for (i = 0; i < CARD_NB_LIGHT; i++)
  {
    if (IsBitSet(conf, i))
    {
      BitNset(Maconf, MesIos[i]);
    }
    else
    {
      BitNclr(Maconf, MesIos[i]);
    }
  }
