...
PRIVATE uint8 showDipSwitch(void)
{
  ...
  // lecture des ios si pas pression sur sw1
  // mode routeur, sinon mode test sorties
...
#endif
  vPrintf("Val1 = %x\n", val);
  val = val & (E_JPI_DIO9_INT | E_JPI_DIO10_INT);
  vPrintf("Val2 = %x\n", val);
  ...
  PBAR_MemoriserPioIdDesRelais();
  PBAR_MontrerPioIdDesEntrees();
