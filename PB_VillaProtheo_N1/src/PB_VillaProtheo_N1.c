...
PRIVATE void PBAR_LireEntree(void);
//PRIVATE bool_t bRoutageDemande(MAC_ExtAddr_s *psAddr);
...
PUBLIC void vJenie_CbConfigureNetwork(void)
{
	...
	//vApi_RegNwkAuthoriseCallback(bRoutageDemande);
...
PUBLIC void vJenie_CbMain(void)
{
	...
	switch(sAppData.eAppState)
	{
		...
		case APP_STATE_TRAITER_INPUT_MESSAGE:
		{
			...
			switch(bufReception[pBuff[1]]){
				...
				case E_MSG_DATA_SELECT:
				{
					...
					vAHI_DioSetOutput(etatSorties<<11,(~etatSorties)<<11);
		...
		case APP_STATE_SET_MY_OUTPUT:
		{
			...
			vAHI_DioSetOutput(config<<11,~config<<11);
			// Mettre a 1 SIG_LE 573 pour maintenir bus
			vAHI_DioSetOutput(C_SEL_573,0);
			...
			//vAHI_DioSetOutput(E_JPI_DIO19_INT,0);
	...
	else
	{
		...
		if(TimePgmPressed)
		{
			...
			if (TimePgmPressed<30)
			{
				...
				rf=0xFF;
				if(!pass){
					vAHI_DioSetOutput(rf<<11,(~rf)<<11); // On
				}
				else
				{
					vAHI_DioSetOutput(~rf<<11,rf<<11);
					rf = 0;
				}
				pass = !pass;
			}
...
PRIVATE void PBAR_LireEntree(void)
{
	uint32 val =0;
	uint8 conf;
	PRIVATE uint8 prev = 0;
	PRIVATE bool_t start = TRUE;

	val = u32AHI_DioReadInput();

	conf = ((uint8)((val>>6)&0x0C) |((uint8)val&0x03));
	if(start==TRUE){
		start = FALSE;
		prev = conf;
		uThisBox_Id = prev;
