...
/****************************************************************************/
PRIVATE void CLAV_ConfigureIO(void);
...
PRIVATE bool_t BC_aConfigurer = FALSE;
...
PUBLIC void vJenie_CbInit(bool_t bWarmStart)
{
	...
	vUtils_Debug("Jenie Init Routeur (enddevice)");
	vPrintf("nb Boites:%d\n",eeprom.nbBoite);
...
PUBLIC void vJenie_CbMain(void)
{
	...
	case APP_STATE_NETWORK_UP:
	{
		...
		vPrintf("> Autorisation association\n\n");
	...
	case APP_STATE_REMOVE_SVC:
	{
		...
		eStatus = eJenie_RegisterServices(0UL);
		vPrintf("   Status = %x\n",eStatus);

		switch (eStatus)
		{
		case E_JENIE_SUCCESS:
			vPrintf(" De-Enregistrement rapide ok\n");
			sAppData.eAppState = APP_STATE_RUNNING;
			break;
		case E_JENIE_DEFERRED:
			vPrintf("    PATIENCE je transmets le De-enregistrement\n");
	...
	case APP_STATE_RUNNING:
	{
		...
		{
			// Indiquer reset possible sur la led
			au8Led[0].mode=E_FLASH_RESET_POSSIBLE;
		}

		if(TimingIo_20>C_IMPULSION_RESET_OUT)
		{
			// indiquer erase + reset
			au8Led[0].mode=E_FLASH_ERASE_RESET_POSSIBLE;
		}
